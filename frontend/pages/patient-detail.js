/**
 * patient-detail.js — page logic for patient-detail.html
 *
 * Extracted from the inline <script type="module"> block by
 * scripts/extract-inline-scripts.mjs to satisfy the strict CSP
 * (script-src 'self') declared in vercel.json. Loaded by the page
 * via <script type="module" src="./patient-detail.js"></script>.
 *
 * Depends on window.__UELFY_CONFIG__ being populated by
 * assets/js/public-config.js, which the page MUST include with a
 * non-module <script> tag BEFORE this module.
 */

import { api, requireAuth, supabase } from '../assets/js/api-client.js';
import { mountNavHeader, computeAgeFromBirthDate } from '../components/nav-header.js';
import { mountProgressCharts } from '../components/progress-charts.js';

await requireAuth();

const escapeHtml = (s) => String(s ?? '').replace(/[&<>"']/g, (c) => ({
  '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;'
})[c]);

// ────────────────────────────────────────────────────────────────────
// Guideline rendering (WS6)
//
// Mirrors the helper in `assessment-view.html`. The server attaches a
// structured `guideline: PublicGuidelineRef` alongside the legacy
// `guidelineSource` displayString when the item maps to a row in the
// shared catalog (see `backend/src/domain/clinical/guideline-catalog/`).
// Off-catalog rows keep the raw string unchanged — nothing regresses.
// ────────────────────────────────────────────────────────────────────
function formatEvidenceLabel(level) {
  if (!level) return '';
  if (level === 'A' || level === 'B' || level === 'C') return `Evidence ${level}`;
  if (level === 'consensus') return 'Consensus';
  if (level === 'policy') return 'Internal policy';
  return String(level);
}
function guidelineCellHtml(item) {
  const g = item?.guideline;
  const raw = item?.guidelineSource ?? '';
  const baseLabel = raw ? escapeHtml(String(raw)) : '';
  if (g && typeof g === 'object') {
    const ev = formatEvidenceLabel(g.evidenceLevel);
    const titleAttr = g.title ? ` title="${escapeHtml(g.title)}"` : '';
    const evChip = ev
      ? `<span class="badge muted" style="margin-left:6px;"${titleAttr}>${escapeHtml(ev)}</span>`
      : '';
    const link = g.url
      ? ` <a href="${escapeHtml(g.url)}" target="_blank" rel="noopener" style="font-size:0.85em;">link ↗</a>`
      : '';
    return `${baseLabel}${evChip}${link}`;
  }
  return baseLabel;
}
function guidelineSourceLineHtml(item) {
  const inner = guidelineCellHtml(item);
  if (!inner) return '';
  return `<div class="muted mt-4" style="font-size:0.8em;">Source: ${inner}</div>`;
}

const params = new URLSearchParams(window.location.search);
const patientId = params.get('id');
if (!patientId || !/^[0-9a-fA-F-]{36}$/.test(patientId)) {
  document.body.innerHTML = '<main class="app-main"><div class="inline-alert danger">Invalid patient id.</div></main>';
  throw new Error('invalid id');
}

/**
 * Shared state consumed by the nav-header renderer. Both
 * `renderPatient` and `renderAssessments` refresh this object and
 * call `renderNavHeader()` so that the breadcrumb appears as soon
 * as possible, and the patient chip + risk dot fill in once the
 * corresponding data lands.
 */
const navState = {
  patient: null,
  latestRiskLevel: null,
};
function renderNavHeader() {
  const mount = document.getElementById('nav-header-mount');
  if (!mount) return;
  const p = navState.patient;
  const chipRef = p?.external_code || (patientId ? patientId.slice(0, 8) : '—');
  mountNavHeader({
    container: mount,
    crumbs: [
      { label: 'Dashboard', href: './dashboard.html' },
      { label: 'Patients',  href: './patients.html' },
      { label: chipRef },
    ],
    backHref: './patients.html',
    backLabel: 'Back to patients',
    patient: {
      id: patientId,
      displayName:
        p?.display_name ||
        [p?.first_name, p?.last_name].filter(Boolean).join(' ') ||
        '—',
      externalCode: p?.external_code || null,
      riskLevel: navState.latestRiskLevel,
      sex: p?.sex || null,
      age: computeAgeFromBirthDate(p?.birth_date ?? p?.birth_year),
    },
  });
}
// Render a skeleton breadcrumb immediately so the header doesn't pop in.
renderNavHeader();

const errorBanner = document.getElementById('error-banner');
function showError(msg) {
  errorBanner.textContent = msg;
  errorBanner.classList.remove('hidden');
}

// Role gating
try {
  const { user } = await api.me();
  if (user.role === 'tenant_admin' || user.role === 'platform_admin') {
    document.getElementById('nav-audit').classList.remove('hidden');
  }
  if (!['clinician', 'tenant_admin', 'platform_admin'].includes(user.role)) {
    document.getElementById('new-assessment-btn').style.display = 'none';
    document.getElementById('new-assessment-btn-2').style.display = 'none';
  }
} catch (e) { console.error(e); }

// Wire new-assessment buttons
const newUrl = `./assessment-new.html?patientId=${encodeURIComponent(patientId)}`;
document.getElementById('new-assessment-btn').href = newUrl;
document.getElementById('new-assessment-btn-2').href = newUrl;

// Export (GDPR Art.15 portability) — uses signed URL via fetch helper
document.getElementById('export-btn').addEventListener('click', async (e) => {
  e.preventDefault();
  try {
    const { data: sess } = await supabase.auth.getSession();
    const token = sess?.session?.access_token;
    if (!token) return;
    const res = await fetch(`/api/v1/patients/${encodeURIComponent(patientId)}/export`, {
      method: 'GET',
      headers: { 'Authorization': `Bearer ${token}` },
    });
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      showError(`Export failed: ${err?.error?.message || res.statusText}`);
      return;
    }
    const blob = await res.blob();
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `patient-${patientId}-export.json`;
    a.click();
    URL.revokeObjectURL(url);
  } catch (err) {
    showError(err.message || 'Export failed');
  }
});

async function renderPatient() {
  try {
    const { patient } = await api.getPatient(patientId);
    navState.patient = patient;
    renderNavHeader();
    document.getElementById('patient-name').textContent =
      patient.display_name || [patient.first_name, patient.last_name].filter(Boolean).join(' ') || '—';
    document.getElementById('patient-subline').textContent =
      `Ref: ${patient.external_code ?? '—'}`;

    document.getElementById('d-code').textContent = patient.external_code ?? '—';
    document.getElementById('d-sex').textContent = patient.sex ?? '—';
    document.getElementById('d-dob').textContent = patient.birth_date ?? (patient.birth_year ? String(patient.birth_year) : '—');
    document.getElementById('d-status').innerHTML = patient.is_active === false
      ? '<span class="badge muted">inactive</span>'
      : '<span class="badge ok">active</span>';
    document.getElementById('d-consent').textContent = patient.consent_status ?? '—';
    document.getElementById('d-created').textContent = patient.created_at
      ? new Date(patient.created_at).toLocaleString()
      : '—';
  } catch (e) {
    showError(`Patient load failed: ${e.message}`);
  }
}

/**
 * Memoise the paginated assessment list so that `renderAssessments` and
 * `renderCompletenessNotices` do not issue two concurrent GETs against
 * the same endpoint. Promise-level memo → safe under `Promise.all`.
 */
let assessmentsListPromise = null;
function getAssessmentsList() {
  if (!assessmentsListPromise) {
    assessmentsListPromise = api
      .listAssessments(patientId, { page: 1, pageSize: 20 })
      .catch((e) => {
        // Reset so a later retry (e.g. completeness) can re-fetch.
        assessmentsListPromise = null;
        throw e;
      });
  }
  return assessmentsListPromise;
}

/**
 * Memoise the latest assessment SNAPSHOT. Multiple cards on this page
 * (completeness notices, lifestyle profile) need the same snapshot; we
 * share a single GET so the page does not re-fetch the (potentially
 * large) assessment payload twice.
 *
 * Resolves to `{ latest, snapshot }` or `{ latest: null, snapshot: null }`
 * when no assessments exist. Keeps the per-consumer code path simple.
 */
let latestSnapshotPromise = null;
function getLatestAssessmentSnapshot() {
  if (!latestSnapshotPromise) {
    latestSnapshotPromise = (async () => {
      const { assessments } = await getAssessmentsList();
      const latest = Array.isArray(assessments) && assessments.length > 0
        ? assessments[0]
        : null;
      if (!latest) return { latest: null, snapshot: null };
      const { snapshot } = await api.getAssessment(latest.id);
      return { latest, snapshot: snapshot ?? null };
    })().catch((e) => {
      latestSnapshotPromise = null;
      throw e;
    });
  }
  return latestSnapshotPromise;
}

async function renderAssessments() {
  const wrap = document.getElementById('assessments-wrap');
  try {
    const { assessments } = await getAssessmentsList();
    // Surface the latest composite risk level on the nav chip. The
    // assessment endpoint returns rows sorted newest-first, so [0] is
    // authoritative for "current" risk stratification.
    navState.latestRiskLevel =
      assessments?.[0]?.riskProfile?.compositeRiskLevel ?? null;
    renderNavHeader();
    if (!assessments?.length) {
      wrap.innerHTML = `<p class="muted">No assessments recorded yet.</p>`;
      return;
    }
    const rows = assessments.map((a) => {
      const rp = a.riskProfile || {};
      const riskBadge = rp.compositeRiskLevel
        ? `<span class="badge ${escapeHtml(rp.compositeRiskLevel)}">${escapeHtml(rp.compositeRiskLevel)}</span>`
        : '<span class="muted">—</span>';
      return `
        <tr>
          <td>${escapeHtml(a.assessmentDate ?? a.createdAt ?? '—')}</td>
          <td>${escapeHtml(a.status ?? '—')}</td>
          <td>${riskBadge}</td>
          <td class="mono">${escapeHtml(a.engineVersion ?? '—')}</td>
          <td><a href="./assessment-view.html?id=${encodeURIComponent(a.id)}">Open</a></td>
        </tr>`;
    }).join('');
    wrap.innerHTML = `
      <table class="table">
        <thead>
          <tr><th>Date</th><th>Status</th><th>Composite risk</th><th>Engine</th><th></th></tr>
        </thead>
        <tbody>${rows}</tbody>
      </table>`;
  } catch (e) {
    wrap.innerHTML = `<div class="inline-alert danger">Failed to load assessments: ${escapeHtml(e.message)}</div>`;
  }
}

/**
 * Longitudinal progress — per-domain enhanced sparklines (WS2).
 *
 * Rendering is delegated to `progress-charts.js`, which owns the
 * SVG layout, threshold-band shading, axis labels, domain colour,
 * and reference legend. This wrapper is kept thin because:
 *   - The chart component has no I/O — it is unit-testable offline.
 *   - Any future swap (e.g. Canvas renderer for very long series)
 *     is a one-import change here.
 *   - The page remains responsible only for fetching + error UX +
 *     subline bookkeeping.
 */
async function renderProgress() {
  const wrap = document.getElementById('progress-wrap');
  const subline = document.getElementById('progress-subline');
  try {
    const trends = await api.getPatientTrends(patientId);
    const { assessmentCount } = mountProgressCharts({
      container: wrap,
      trends,
    });
    subline.textContent = assessmentCount > 0
      ? `${assessmentCount} assessment${assessmentCount === 1 ? '' : 's'} plotted`
      : '—';
  } catch (e) {
    wrap.innerHTML = `<div class="inline-alert danger">Failed to load progress charts: ${escapeHtml(e.message)}</div>`;
    subline.textContent = '—';
  }
}

/**
 * Due items — materialised follow-up / screening countdown (WS7).
 *
 * - Server returns `nowIso` for monotonic countdown across clock drift.
 * - Items are grouped by priority (urgent → moderate → routine) and
 *   sorted within each group by `dueInDays` ascending so overdue items
 *   bubble to the top.
 * - Dismissed / completed items are filtered server-side by default.
 */
function dueInLabel(days) {
  if (typeof days !== 'number' || Number.isNaN(days)) return '—';
  if (days < 0) return `Overdue ${Math.abs(days)} day${Math.abs(days) === 1 ? '' : 's'}`;
  if (days === 0) return 'Due today';
  if (days === 1) return 'Due in 1 day';
  if (days < 30) return `Due in ${days} days`;
  const months = Math.round(days / 30);
  return months === 1 ? 'Due in ~1 month' : `Due in ~${months} months`;
}

/**
 * Map the canonical `countdownStatus` (computed server-side) to the UI
 * badge class. Keeping this pure of `dueInDays` guarantees the UI and
 * the server agree on the bucketing even if client clocks drift.
 */
function countdownBadgeClass(countdownStatus, priority) {
  if (countdownStatus === 'overdue') return 'danger';
  if (countdownStatus === 'due_now') return 'danger';
  if (countdownStatus === 'due_soon') return 'warning';
  if (priority === 'urgent') return 'danger';
  if (priority === 'moderate') return 'warning';
  return 'ok';
}

/** Human label for the canonical status — shown as a small secondary pill. */
function countdownStatusLabel(countdownStatus) {
  switch (countdownStatus) {
    case 'overdue':  return 'overdue';
    case 'due_now':  return 'due now';
    case 'due_soon': return 'due soon';
    case 'upcoming': return 'upcoming';
    default:         return '';
  }
}

async function renderDueItems() {
  const wrap = document.getElementById('due-items-wrap');
  const subline = document.getElementById('due-items-subline');
  try {
    const { items, summary, pagination } = await api.listPatientDueItems(patientId, {
      // Repeated keys so server-side union (enum | array) matches.
      status: ['open', 'acknowledged'],
      pageSize: 100,
    });
    if (!items?.length) {
      wrap.innerHTML = `<p class="muted">No pending follow-up or screening items.</p>`;
      subline.textContent = '0 pending';
      return;
    }
    // Trust the server `summary` (canonical bucketing from the
    // countdown-status model). Fallback to a client-side recount
    // only if the field is missing (e.g. older backend).
    const total = summary?.total ?? pagination?.total ?? items.length;
    const overdue = summary?.overdue ?? items.filter(
      (it) => it.countdownStatus === 'overdue'
        || (typeof it.dueInDays === 'number' && it.dueInDays < 0),
    ).length;
    const dueNow = summary?.dueNow ?? items.filter(
      (it) => it.countdownStatus === 'due_now',
    ).length;
    const parts = [`${total} pending`];
    if (overdue > 0) parts.push(`${overdue} overdue`);
    if (dueNow > 0) parts.push(`${dueNow} due now`);
    subline.textContent = parts.join(' · ');

    const priorityOrder = { urgent: 0, moderate: 1, routine: 2 };
    const groups = { urgent: [], moderate: [], routine: [] };
    for (const it of items) {
      const p = groups[it.priority] ? it.priority : 'routine';
      groups[p].push(it);
    }
    for (const p of Object.keys(groups)) {
      groups[p].sort((a, b) => {
        const da = typeof a.dueInDays === 'number' ? a.dueInDays : Number.MAX_SAFE_INTEGER;
        const db = typeof b.dueInDays === 'number' ? b.dueInDays : Number.MAX_SAFE_INTEGER;
        return da - db;
      });
    }

    const renderGroup = (label, list) => {
      if (!list.length) return '';
      const rows = list.map((it) => {
        const days = it.dueInDays;
        const label2 = dueInLabel(days);
        const status = it.countdownStatus
          || (typeof days === 'number' && days < 0 ? 'overdue' : 'upcoming');
        const badgeCls = countdownBadgeClass(status, it.priority);
        const statusPill = status
          ? `<span class="badge muted" style="margin-left:6px;">${escapeHtml(countdownStatusLabel(status))}</span>`
          : '';
        const sourceBadge = it.sourceEngine
          ? `<span class="badge muted" style="margin-left:6px;">${escapeHtml(it.sourceEngine)}</span>`
          : '';
        const domainBadge = it.domain
          ? `<span class="badge muted" style="margin-left:6px;">${escapeHtml(it.domain)}</span>`
          : '';
        const rationale = it.rationale
          ? `<div class="muted mt-4" style="font-size:0.9em;">${escapeHtml(it.rationale)}</div>`
          : '';
        const guideline = guidelineSourceLineHtml(it);
        return `
          <li class="mt-8" style="padding:10px 12px;border:1px solid var(--border,#e5e7eb);border-radius:6px;">
            <div class="flex between" style="align-items:flex-start;">
              <div>
                <strong>${escapeHtml(it.title)}</strong>
                ${statusPill}${sourceBadge}${domainBadge}
                ${rationale}
                ${guideline}
              </div>
              <div style="text-align:right;white-space:nowrap;">
                <span class="badge ${escapeHtml(badgeCls)}">${escapeHtml(label2)}</span>
                <div class="muted mt-4" style="font-size:0.8em;">Due ${escapeHtml(it.dueAt ?? '—')}</div>
              </div>
            </div>
          </li>`;
      }).join('');
      return `
        <div class="mt-16">
          <div class="kpi-label">${escapeHtml(label)} (${list.length})</div>
          <ul class="list-plain">${rows}</ul>
        </div>`;
    };

    wrap.innerHTML = [
      renderGroup('Urgent', groups.urgent),
      renderGroup('Moderate', groups.moderate),
      renderGroup('Routine', groups.routine),
    ].filter(Boolean).join('') || `<p class="muted">No pending items.</p>`;
  } catch (e) {
    wrap.innerHTML = `<div class="inline-alert danger">Failed to load due items: ${escapeHtml(e.message)}</div>`;
    subline.textContent = '—';
  }
}

/**
 * Open clinical alerts (WS8).
 *
 * The alert engine has been split so that `missing_critical_data`
 * belongs to the separate completeness stream (see
 * `completeness-checker.ts`). We defensively filter any legacy rows of
 * that type out of the inbox on the UI too, so a historical write can
 * never re-pollute the clinical alerts inbox.
 */
async function renderAlerts() {
  const wrap = document.getElementById('alerts-wrap');
  const subline = document.getElementById('alerts-subline');
  try {
    const { alerts } = await api.listPatientAlerts(patientId, { status: 'open', pageSize: 20 });
    const clinical = (alerts ?? []).filter(
      (a) => a?.type !== 'missing_critical_data',
    );
    subline.textContent = clinical.length
      ? `${clinical.length} open`
      : '0 open';
    if (!clinical.length) {
      wrap.innerHTML = `<p class="muted">No open alerts.</p>`;
      return;
    }
    wrap.innerHTML = `
      <ul class="list-plain">
        ${clinical.map((a) => `
          <li>
            <span class="badge ${escapeHtml(a.severity)}">${escapeHtml(a.severity)}</span>
            <strong>${escapeHtml(a.title ?? a.type ?? 'Alert')}</strong>
            <span class="muted"> · ${escapeHtml(a.message ?? '')}</span>
            <div class="muted mt-8">Opened: ${escapeHtml(new Date(a.created_at).toLocaleString())}</div>
          </li>
        `).join('')}
      </ul>`;
  } catch (e) {
    wrap.innerHTML = `<div class="inline-alert danger">Failed to load alerts: ${escapeHtml(e.message)}</div>`;
    subline.textContent = '—';
  }
}

/**
 * Completeness notices (WS8 — data-collection punch list).
 *
 * Sourced from the most recent assessment snapshot's
 * `completenessWarnings` array. Kept explicitly separate from the
 * clinical alerts inbox because these are soft nudges, not time-bound
 * actions. Severity maps:
 *   - warning → yellow "needed" badge
 *   - info    → neutral "optional" badge
 *   - critical is never produced by the checker (see module doc).
 */
function completenessBadge(severity) {
  if (severity === 'warning') return '<span class="badge warning">needed</span>';
  return '<span class="badge muted">optional</span>';
}

async function renderCompletenessNotices() {
  const wrap = document.getElementById('completeness-wrap');
  const subline = document.getElementById('completeness-subline');
  try {
    const { latest, snapshot } = await getLatestAssessmentSnapshot();
    if (!latest) {
      wrap.innerHTML = `<p class="muted">No assessments on file — completeness will be evaluated once the first assessment is recorded.</p>`;
      subline.textContent = '—';
      return;
    }
    const warnings = Array.isArray(snapshot?.completenessWarnings)
      ? snapshot.completenessWarnings
      : [];
    const asOf = latest.assessmentDate || latest.createdAt || '';
    if (!warnings.length) {
      wrap.innerHTML = `
        <p class="muted">
          No missing-data gaps as of the most recent assessment
          ${asOf ? `(${escapeHtml(asOf)})` : ''}.
        </p>`;
      subline.textContent = 'up to date';
      return;
    }

    // Order: warnings first (directly in scope), then info.
    const ordered = [...warnings].sort((a, b) => {
      const rank = (s) => (s === 'warning' ? 0 : s === 'info' ? 1 : 2);
      return rank(a.severity) - rank(b.severity);
    });

    const warningCount = ordered.filter((w) => w.severity === 'warning').length;
    subline.textContent = warningCount > 0
      ? `${ordered.length} gaps · ${warningCount} needed`
      : `${ordered.length} optional gaps`;

    wrap.innerHTML = `
      <div class="muted mt-4" style="font-size:0.8em;">
        As of assessment ${escapeHtml(asOf || '—')}.
      </div>
      <ul class="list-plain mt-8">
        ${ordered.map((w) => {
          const missing = Array.isArray(w.missingFields) && w.missingFields.length
            ? `<div class="mono muted mt-4" style="font-size:0.8em;">Missing: ${w.missingFields.map(escapeHtml).join(', ')}</div>`
            : '';
          const action = w.suggestedAction
            ? `<div class="muted mt-4" style="font-size:0.85em;"><em>Suggested:</em> ${escapeHtml(w.suggestedAction)}</div>`
            : '';
          return `
            <li class="mt-8" style="padding:10px 12px;border:1px solid var(--border,#e5e7eb);border-radius:6px;">
              <div class="flex between" style="align-items:flex-start;gap:12px;">
                <div>
                  <strong>${escapeHtml(w.title ?? w.code ?? 'Missing data')}</strong>
                  <span class="mono muted" style="margin-left:6px;font-size:0.8em;">${escapeHtml(w.code ?? '')}</span>
                  <div class="muted mt-4" style="font-size:0.9em;">${escapeHtml(w.detail ?? '')}</div>
                  ${missing}
                  ${action}
                </div>
                <div style="white-space:nowrap;">
                  ${completenessBadge(w.severity)}
                </div>
              </div>
            </li>`;
        }).join('')}
      </ul>`;
  } catch (e) {
    wrap.innerHTML = `<div class="inline-alert danger">Failed to load completeness notices: ${escapeHtml(e.message)}</div>`;
    subline.textContent = '—';
  }
}

/**
 * Lifestyle profile (ISSUE 3 — patient-overview surfacing).
 *
 * Pulls the most recent assessment snapshot (memoised, shared with the
 * completeness card) and renders a compact view of:
 *   - PREDIMED MEDAS score + adherence band
 *   - Physical-activity volume (MET-min/wk) + WHO 2020 status
 *   - Sedentary risk band
 *   - Smoking flag
 *   - Top 3 supportive lifestyle recommendations (priority-ordered)
 *
 * "Recommendations" come straight from the bounded WS6 engine, so the
 * card is byte-equivalent to the assessment-view section but without
 * the prescriptive framing. Each row carries the engine's guideline
 * citation and a `supportive` micro-label so the user understands the
 * authority boundary.
 */
function priorityBadge(priority) {
  const p = String(priority ?? '').toLowerCase();
  if (p === 'urgent') return '<span class="badge danger">urgent</span>';
  if (p === 'moderate') return '<span class="badge warning">moderate</span>';
  return '<span class="badge muted">routine</span>';
}

function adherenceBadge(band) {
  if (band === 'high') return '<span class="badge ok">high</span>';
  if (band === 'medium') return '<span class="badge warning">medium</span>';
  if (band === 'low') return '<span class="badge danger">low</span>';
  return '<span class="badge muted">—</span>';
}

function whoStatusBadge(meets) {
  if (meets === true)  return '<span class="badge ok">met</span>';
  if (meets === false) return '<span class="badge warning">not met</span>';
  return '<span class="badge muted">—</span>';
}

function sedentaryBadge(level) {
  const v = String(level ?? '').toLowerCase();
  if (v === 'very_high') return '<span class="badge danger">very high</span>';
  if (v === 'high')      return '<span class="badge warning">high</span>';
  if (v === 'moderate')  return '<span class="badge warning">moderate</span>';
  if (v === 'low')       return '<span class="badge ok">low</span>';
  return '<span class="badge muted">—</span>';
}

async function renderLifestyleProfile() {
  const wrap = document.getElementById('lifestyle-wrap');
  const subline = document.getElementById('lifestyle-subline');
  try {
    const { latest, snapshot } = await getLatestAssessmentSnapshot();
    if (!latest || !snapshot) {
      wrap.innerHTML = `<p class="muted">No assessments on file — lifestyle profile will be available once the first assessment is recorded.</p>`;
      subline.textContent = '—';
      return;
    }

    const asOf = latest.assessmentDate || latest.createdAt || '';
    const nutrition = snapshot.nutritionSummary || {};
    const activity  = snapshot.activitySummary  || {};
    const smokingFlag = snapshot?.input?.clinicalContext?.smoking === true;
    const recommendations = Array.isArray(snapshot.lifestyleRecommendations)
      ? snapshot.lifestyleRecommendations
      : [];

    // Sort priority urgent → moderate → routine, stable within each.
    const priRank = (p) => (p === 'urgent' ? 0 : p === 'moderate' ? 1 : 2);
    const ordered = [...recommendations].sort((a, b) => priRank(a.priority) - priRank(b.priority));
    const top = ordered.slice(0, 3);
    const remaining = ordered.length - top.length;

    const predimed = nutrition.predimedScore;
    const predimedCell = predimed != null
      ? `<strong>${escapeHtml(String(predimed))}/14</strong> ${adherenceBadge(nutrition.adherenceBand)}`
      : '<span class="muted">—</span>';

    const metMin = activity.metMinutesPerWeek;
    const minsWeek = activity.minutesPerWeek;
    const metaPieces = [];
    if (metMin != null) metaPieces.push(`${Math.round(metMin)} MET-min/wk`);
    if (minsWeek != null) metaPieces.push(`${minsWeek} min/wk`);
    const activityCell = metaPieces.length
      ? `<strong>${escapeHtml(metaPieces.join(' · '))}</strong> ${whoStatusBadge(activity.meetsWhoGuidelines)}`
      : `<span class="muted">—</span> ${whoStatusBadge(activity.meetsWhoGuidelines)}`;

    const sedentaryCell = sedentaryBadge(activity.sedentaryRiskLevel);
    const smokingCell = smokingFlag
      ? '<span class="badge danger">current smoker</span>'
      : '<span class="badge ok">non-smoker</span>';

    const summaryCount = recommendations.length;
    subline.textContent = summaryCount > 0
      ? `${summaryCount} recommendation${summaryCount === 1 ? '' : 's'}`
      : 'no nudges';

    const recsHtml = top.length === 0
      ? `<p class="muted mt-8">
           No lifestyle nudges from the most recent assessment — current
           profile is within the engine's supportive thresholds.
         </p>`
      : `
        <ul class="list-plain mt-8">
          ${top.map((r) => {
            const sourceLine = guidelineSourceLineHtml(r);
            return `
              <li class="mt-8" style="padding:10px 12px;border:1px solid var(--border,#e5e7eb);border-radius:6px;">
                <div class="flex between" style="align-items:flex-start;gap:12px;">
                  <div>
                    <strong>${escapeHtml(r.title ?? r.code ?? 'Recommendation')}</strong>
                    <span class="badge muted" style="margin-left:6px;">supportive</span>
                    <div class="muted mt-4" style="font-size:0.9em;">${escapeHtml(r.rationale ?? '')}</div>
                    ${sourceLine}
                  </div>
                  <div style="white-space:nowrap;">
                    ${priorityBadge(r.priority)}
                  </div>
                </div>
              </li>`;
          }).join('')}
        </ul>
        ${remaining > 0
          ? `<div class="muted mt-8" style="font-size:0.85em;">
               + ${remaining} additional recommendation${remaining === 1 ? '' : 's'} —
               <a href="./assessment-view.html?id=${encodeURIComponent(latest.id)}">open the latest assessment</a>
               for the full list.
             </div>`
          : ''}
      `;

    wrap.innerHTML = `
      <div class="muted mt-4" style="font-size:0.8em;">
        As of assessment ${escapeHtml(asOf || '—')}.
      </div>
      <div class="grid two mt-8" style="gap:8px 16px;">
        <div><div class="kpi-label">PREDIMED (MEDAS)</div><div>${predimedCell}</div></div>
        <div><div class="kpi-label">Physical activity</div><div>${activityCell}</div></div>
        <div><div class="kpi-label">Sedentary risk</div><div>${sedentaryCell}</div></div>
        <div><div class="kpi-label">Smoking</div><div>${smokingCell}</div></div>
      </div>
      ${recsHtml}
    `;
  } catch (e) {
    wrap.innerHTML = `<div class="inline-alert danger">Failed to load lifestyle profile: ${escapeHtml(e.message)}</div>`;
    subline.textContent = '—';
  }
}

async function renderConsents() {
  const wrap = document.getElementById('consents-wrap');
  try {
    const { consents } = await api.listConsents(patientId);
    if (!consents?.length) {
      wrap.innerHTML = `<p class="muted">No consent records.</p>`;
      return;
    }
    wrap.innerHTML = `
      <table class="table">
        <thead>
          <tr><th>Type</th><th>Status</th><th>Policy version</th><th>Recorded</th></tr>
        </thead>
        <tbody>
          ${consents.map((c) => `
            <tr>
              <td>${escapeHtml(c.consent_type)}</td>
              <td>${c.granted
                ? '<span class="badge ok">granted</span>'
                : '<span class="badge muted">revoked</span>'}</td>
              <td class="mono">${escapeHtml(c.policy_version ?? '—')}</td>
              <td>${escapeHtml(new Date(c.recorded_at ?? c.created_at).toLocaleString())}</td>
            </tr>`).join('')}
        </tbody>
      </table>`;
  } catch (e) {
    wrap.innerHTML = `<div class="inline-alert danger">Failed to load consents: ${escapeHtml(e.message)}</div>`;
  }
}

await Promise.all([
  renderPatient(),
  renderAssessments(),
  renderProgress(),
  renderLifestyleProfile(),
  renderDueItems(),
  renderAlerts(),
  renderCompletenessNotices(),
  renderConsents(),
]);

document.getElementById('signout-link').addEventListener('click', async (e) => {
  e.preventDefault();
  await api.signOut();
  window.location.href = './login.html';
});
