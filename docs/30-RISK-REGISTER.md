...
>
> **Updated.** 2026-05-11 (Sprint 7 close).
...
|---|---|---|---|
| H-01 | RLS regression undetected (no Postgres-side test) | ✅ Resolved (substantive enforcement Sprint 7 task 7.2) | `tests/rls/cross_tenant_negative.sql` (Tier 1, Task #68) — 6 SQL assertions exercise the live policy set with `SET LOCAL ROLE authenticated` + JWT-claim impersonation. Wrapped in BEGIN…ROLLBACK so re-runnable. Wire: `npm run test:rls`. **Sprint 7 elevation**: `scripts/run-rls-tests.mjs` is now **fail-closed in CI** when `DATABASE_URL_STAGING` GH secret is non-empty (skip-graceful only in local dev or during the rollout window before staging is provisioned). `.github/workflows/ci.yml` exposes the secret at job level on `build-and-test`. A `Verify psql is available` step blocks the silent-SKIP failure mode of a regressed runner image. `RLS_GATE_REQUIRED=1` env opt-in for paranoid local testing. Pending: operator runs `docs/35-RLS-STAGING-RUNBOOK.md` §2 to provision staging + set the secret. |
...
| C-01 | SCORE2 calibration formula divergent from paper (cll vs shortcut) | ✅ Resolved | Tier-5 / AUD-2026-05-04: production engine in `score2.ts` + `score2-diabetes.ts` now implements the canonical Hageman 2021 Box S5 complementary log-log recalibration. Independent reference implementation in `tests/unit/score2-golden.test.ts` (9 sex × region × age cases) cross-checks production output to within ±0.1 %. Five regression assertions catch drift back to the shortcut form (male S0 hard-coded, missing sex-specific calibration, missing cll transform). External confirmation against ESC HeartScore web tool recommended for clinical sign-off but no longer a technical blocker. Numeric impact: e.g. M, 62y, smoker, SBP 168 mmHg, TC 251 mg/dL went from 11.7 % (shortcut, mis-classified "high") to 21.0 % (canonical, correctly "very high"). Engine version bumped accordingly |
| C-04-PRED | PREDIMED MEDAS adherence band thresholds non-canonical | ✅ Resolved | Tier-5: bands aligned to Schroder 2011 / Estruch 2018: ≤7 low / 8-9 medium / ≥10 high (was 0-4 / 5-9 / 10-14). Formula (count of yes answers) unchanged. Golden test `tests/unit/predimed-mifflin.test.ts` covers all band boundaries + invalid input + truncation + integration with Mifflin BMR/TDEE |
...
| M-02 | Tenant-admin UI for retention overrides incomplete | ✅ Resolved | Migration `014_tenant_retention_overrides.sql` adds 4 nullable INTEGER columns to `tenants` (audit, anonymize_grace, alerts_resolved, notifications) with sensible CHECK bounds. Endpoint `/api/v1/admin/tenant` GET+PATCH (Tier 3, Task #74) — tenant_admin scoped to own tenant, platform_admin via `?id=`. PATCH uses `recordAuditStrict` (B-09) so a privacy-significant change to retention windows can never escape the audit trail. UI: `frontend/pages/tenant-settings.html` + `tenant-settings.js`. **Closed in Tier 4 (migration 015)**: `fn_retention_prune` now reads `tenants.retention_days_*` via `COALESCE(tenant_value, platform_default)` for audit / alerts / notifications. Cron run report includes a per-tenant breakdown emitted as `RETENTION_RUN` structured event. Migration 016 adds a BRIN index on `audit_events.created_at` so the per-tenant DELETE scales. Partitioned-table cutover SOP documented in `26-DEPLOYMENT-RUNBOOK.md §12b` for the >50M-row regime |
...
| M-07 | PDF visual-regression test absent | 🟢 Mitigated | Tier 4 — `tests/unit/pdf-report-service.test.ts` extended with a 5-case visual-regression suite. Determinism is achieved by injecting a fixed `generatedAt: Date` into the renderer (production callers ignore the option). Suite covers: (a) byte-deterministic output for fixed inputs, (b) byte-length stability inside a tolerance band (BASELINE ± 25 KB), (c) round-trip via `PDFDocument.load` with non-zero page count + Info dict assertions, (d) presence of payload identity markers in the byte stream, (e) "renderer responds to payload size" sanity (alerts ↑ → bytes ↑). Pixel-diff vs baseline image is OUT of scope (would need headless Chromium / Poppler — not Vercel-serverless-friendly). Wired via `npm run test:pdf` |
...
| L-07 | Per-score code-coverage report (informational, not gating) | ✅ Resolved (Sprint 6 task 6.1) **+ CI-enforced (Sprint 7 task 7.1)** | `tests/vitest.config.ts` emits `json-summary` reporter via `@vitest/coverage-v8`; `scripts/check-coverage-thresholds.mjs` enforces per-module floors (validated formulas 85-90 %, interpretation layers 80 %, supporting modules 70 %). Wired into `npm run check:coverage` locally. **Sprint 7 elevation**: dedicated `coverage-gate` job in `.github/workflows/ci.yml` runs in parallel to `build-and-test` on every PR + push to main, fail-closed when any tracked module falls below floor. HTML drill-down uploaded as artifact with 7-day retention. Branch-protection wire-up (require `coverage-gate` green before merge) is operator follow-up — flip after first green run validates the job. |
...
| Low | 10 listed | 2 closed (Sprint 5 L-08 → 5.4; Sprint 6 L-07 → 6.1, **CI-enforced Sprint 7 → 7.1**); 8 remain on roadmap |
