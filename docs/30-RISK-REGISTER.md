...
| L-07 | Per-score code-coverage report (informational, not gating) | ✅ Resolved (Sprint 6 task 6.1) | `tests/vitest.config.ts` now emits `json-summary` reporter via `@vitest/coverage-v8`; `scripts/check-coverage-thresholds.mjs` enforces per-module floors (validated formulas 90 %, interpretation layers 80 %, supporting modules 70 %). Wired into `npm run check:coverage` (which chains `test:coverage` + the gate). Not in `build:check` itself — coverage takes longer than the structural gates and lives in a separate CI step. |
...
| Low | 10 listed | 2 closed (Sprint 5 L-08 → 5.4; Sprint 6 L-07 → 6.1); 8 remain on roadmap |
