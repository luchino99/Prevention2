...
| L-09 | MFA required for all clinician roles | 🟢 Mitigated, role-keyed | Backend gate in `auth-middleware.validateAccessToken` is now driven by a per-role flag matrix (Tier 4 extension): `MFA_ENFORCEMENT_ENABLED` covers `platform_admin`+`tenant_admin` (Tier 2, in prod), `MFA_ENFORCEMENT_CLINICIAN_ENABLED` covers `clinician`, `MFA_ENFORCEMENT_STAFF_ENABLED` covers `assistant_staff`. All flags default-off; operators flip per-role after the corresponding cohort has enrolled at `/pages/mfa-enroll.html`. The Tier 3 mfa-enroll dispatcher is role-agnostic (handles enrol + challenge for every role uniformly), so no frontend change was required. `mfa_required` AccessDenialReason already in place since Tier 1. The patient role is intentionally not gated (controller-side product decision) |
